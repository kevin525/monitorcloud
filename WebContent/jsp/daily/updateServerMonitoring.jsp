 ...
 $(function(){
	 $.ajax({
			...
			success : function(data) {
				...
				$("#personLiable").val(data.personLiable);
				$("#cpu").val(data.cpu);
				$("#osName").val(data.osName);
				$("#ip").val(data.ip);
				$("#memorySize").val(data.memorySize);
				$("#hardDiskSize").val(data.hardDiskSize);
				$("#availableHDSzie").val(data.availableHDSzie);
				$("#serverOrder").val(data.serverOrder);
...
function addSystem(){
		 if(check()){
			 $.ajax({
				type : 'post',
				url : "${configRoot}/server/updateServer.do",
				data : params,
...
<input type="hidden" id="status" />
	<table class="table table-bordered table-hover m10">
		<tr align="center">
			<td class="tableleft" style="height:30px;line-height:30px;width:15%;" align="center"><span style="color: red">*</span>服务器名称</span>
			</td>
			<td width="35%"><input type="text" name="name" id="name"/></td>
			<td class="tableleft" style="height:30px;line-height:30px;width:15%;" align="center"><span style="color: red">*</span>服务器ip</span></td>
		...
		<tr>
			...
			<td style="border-right: 1px solid #dddddd;"><input type="radio" name="environment"  id="environment" value="0"/>正式环境
		...
		<tr>
			<td colspan="2">
			    <button class="layui-btn layui-btn-normal" onclick="addSystem()">确定</button>
