var fBuildLogContentTr_V = function(con,i){
	...
	html += "<td align='center' width='150px'>"+con.port+"</td>" ;
...
function init(){
	oParams.start=0;
	oParams.pageSize=10;
	sUrl=Config.ROOT+"/middleware/getMiddlewareList.do";
...
function getid(id) {
	parent.layer.open({
		...
		shadeClose : true,
		area : [ '900px', '75%' ],
		content : 'jsp/daily/middleware/updateMiddleware.jsp?id=' + id
...
function changeIsUse(sourceStr,isUse){
	...
	params.val = isUse;
	 $.ajax({
			type : 'post',
			url : Config.ROOT+"/middleware/setMiddlewareUse.do",
			data : params,
			success : function(con) {
				if(con.data=="ok"){
					init();
					...
					parent.layer.msg('中间件出错', {icon: 2,time: 500});
				}
			}
		}); 
}
