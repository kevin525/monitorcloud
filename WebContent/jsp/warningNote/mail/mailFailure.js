var fBuildLogContentTr_V = function(con,i){
	...
	html +=	"<td align='left'  style='padding:0 3px; line-height:20px;'>"+con.subject+"</td>";
	html +=	"<td align='left'  style='padding:0 3px; line-height:20px;'>"+con.content+"</td>";
	html += "<td align='left' >"+con.receiveAddress+"</td>" ;
...
function init(){
	oParams.start=0;
	oParams.pageSize=8;
	sUrl=Config.ROOT+"/mail/getMailFailureList.do";
...
function editSystemState(id) {
	parent.layer.open({
		...
		shadeClose : true,
		area : [ '700px', '500px' ],
		content : 'jsp/daily/sms/sms.jsp?id=' + id
	});
};*/
